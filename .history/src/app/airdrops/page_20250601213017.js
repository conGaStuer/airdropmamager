...
export default function AirdropPages() {
  ...
  const apiUrl = "https://airdropmanager.onrender.com";
  const typeClassMap = {
    depin:
  ...
  const [completedAirdrop, setCompletedAirdrop] = useState([]);
  const handleCheckboxChange = (id) => {
    if (completedAirdrop.includes(id)) {
      setCompletedAirdrop(completedAirdrop.filter((itemId) => itemId !== id));
    } else {
      setCompletedAirdrop([...completedAirdrop, id]);
    }
  };
  ...
  const currentItems = filteredAirdrops.slice(
    indexOfFirstItem,
    indexOfLastItem
  );
  ...
  useEffect(() => {
    const stored = localStorage.getItem("completedAirdrop");
    if (stored) setCompletedAirdrop(JSON.parse(stored));
  }, []);

  useEffect(() => {
    localStorage.setItem("completedAirdrop", JSON.stringify(completedAirdrop));
  }, [completedAirdrop]);
  useEffect(() => {
  ...
  return (
    <div style={{ padding: 20 }}>
      <div className="flex justify-around" style={{ width: 900 }}>
        <button
        ...
        <ExportExcel data={airdrops} />
        <div className="flex flex-wrap gap-4 mb-4 relative ml-4 top-1">
          <div>
            <label className="mr-2 text-sm">Lọc theo Status:</label>
            <select
              value={statusFilter}
          ...
          <div>
            <label className="mr-2 text-sm">Lọc theo Type:</label>
            <select
              value={typeFilter}
        ...
        <input
          ...
          className=" border rounded text-sm w-60 h-8 ml-4 p-3"
      ...
      <div className=" grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        />
        {balance === null ? (
          <p className="text-red-500">Chưa kết nối ví MetaMask</p>
        ) : (
          <StatCard
            ...
            {currentItems.map((item) => (
              <tr key={item.id} className="border-t text-center text-sm">
                <td>
                  <input
                    type="checkbox"
                    checked={completedAirdrop.includes(item.id)}
      ...
      ${
        item.type === "DEPIN"
          ...
          : "text-gray-500 bg-gray-100 border-gray-300 hover:bg-gray-200"
                ...
                <td>
                  <Link href={`/claim/${item.id}`} target="_blank">
